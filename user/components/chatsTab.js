...
const ChatsTab = ({ navigation }) => {
              ...
              source={
                typeof enhancedItem.image === 'number' 
                  ...
                  : typeof enhancedItem.image === 'string' && enhancedItem.image.startsWith('http')
                    ...
                    : require("../assets/images/pic1.png") // Fallback chat avatar
              }
              style={styles.image} 
            />
  ...
  const combinedChatList = () => {
    ...
    // Add dynamic chats that aren't already in static list
    const newChats = dynamicChats.filter(chat => !staticNames.has(chat.name));
