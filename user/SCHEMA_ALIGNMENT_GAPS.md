...
| Profile-directory avatar fields were storing local device URIs instead of persisted storage URLs | Family/daily-help/vehicle/frequent-entry create/edit modals wrote picked `file://` image paths directly into `avatar_url`, while profile surfaces only rendered real remote URLs | Entry photos disappeared after save, cross-device rendering was broken, and the add-photo UX was misleading in production | `Code alignment` (completed: directory entry avatars now upload to storage and profile surfaces fall back to deterministic DiceBear gradient avatars when no custom image exists) |
| User vehicle directory flow referenced non-existent `plate_number` and left gate-pass QR payloads stale after edits | Vehicle create used `vehicle_number`, edit/profile detail expected `plate_number`, and all directory update hooks skipped QR payload regeneration | Vehicle edits could write invalid fields and gate-pass metadata could show pre-edit relation/type/model details | `Code alignment` (completed: user profile directory now uses canonical `vehicle_number` and refreshes QR payloads on update across family/daily-help/vehicle/frequent-entry hooks) |
