...
| Community directory membership sync/validation existed live but was not represented on current `main` | `/Users/andromeda/casanirvana/user/hooks/useCommunityMembers.ts`, `/Users/andromeda/casanirvana/Guard/hooks/useCommunityDirectoryMembers.js` relied on `community_memberships` staying aligned with `profiles.community_id` while repo migrations only showed the initial Phase 10 backfill | Directory rows could drift from profile community assignments on environments that never received the untracked live SQL | `Migration + Code alignment` (completed via `supabase/migrations/20260322153000_phase43_community_directory_membership_integrity_parity.sql` plus `units`-aware directory invalidation and Guard community-scoped recent-search hygiene) |
| Notice read policies were previously over-permissive | Legacy `public.notices` policies included broad read paths (`USING true`) | Tenant scope leakage risk in superadmin/user contexts | `Migration` (completed via `supabase/migrations/20260221004421_phase13_notices_rls_scope_hardening.sql`) |
