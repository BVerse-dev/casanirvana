...
| `messageScreen` | Send/receive messages, attachments, call events | Uses `useMessages`, `useRealTimeMessages`, writes chat files to private storage bucket `chat-attachments` with owner-scoped path (`{auth.uid}/chat/*`), surfaces explicit loading/error/empty states, and opens hydrated signed attachment URLs | `Wired` | Keep; preserve owner-scoped upload path, signed-url attachment reads, and DB message contract (`from_user`/`to_user`, `message_type`, `attachments`) |
| `useMessages` | Message/call fetch + send/update | Reads/writes `messages` and reads `calls`; filters soft-deleted rows, hydrates signed attachment URLs, and handles `read`/`is_read` fallback plus conversation invalidation | `Wired` | Keep; preserve canonical message columns and TanStack v5 object-form invalidation keys |
| `callScreen` + `useCallManager` + `IncomingCallNavigationHandler` | Outgoing/incoming direct-call signaling | Uses realtime `calls` row subscriptions for state, routes inbound calls into `callScreen`, and mutates answer/reject/end against DB status only (no fake transport simulation) | `Wired` | Keep profile-scoped caller/callee ids, live `initiated/ringing/answered/ended` status mapping, and incoming-route handoff |
...
| `services/serviceProviderCatalogService.js` | Provider catalog reads for payment entry screens | Reads active providers through RPC `list_active_service_providers` (safe column subset), applies bill-category filtering (`utilities` / `tv`), and falls back to static catalog only when RPC/data is unavailable | `Wired` | Keep RPC as canonical source; maintain fallback for runtime resilience only |
...
| `airtimeScreen` / `dataScreen` / `transferScreen` / `utilitiesScreen` / `tvScreen` / `insuranceScreen` | Provider selection entry state | Provider lists now load from DB-backed catalog RPC (with resilient fallback), and route payloads include both stable provider code and `providerId` for downstream persistence | `Wired` | Keep DB catalog as source of truth; maintain fallback until all environments have RPC migration |
...
| `billAmountScreen` / `insuranceAmountScreen` | Amount entry + verification card | Removed simulated timeout verification; amount is user-entered and passed through canonical payload contract | `Wired` | Keep; wire provider verification API only when backend integration is available |
| `services/expressPayService.js` | Authenticated backend checkout client | Calls backend `initiate`/`verify`/`status` ExpressPay endpoints with Supabase access token and reconciles terminal status (poll fallback) | `Wired` | Keep as canonical payment-gateway client for user app checkout flows |
| `paymentMethodScreen` and sub-screens | Method selection and checkout transition | Enforces one canonical personal-hub payload contract; card/paypal/mobile-money checkout screens now use hosted ExpressPay flow via backend (no direct client-side payment success writes) | `Wired` | Keep; preserve canonical payload contract across retries/history |
| `mobileMoneyScreen` | Payment initiation + confirmation modal | Initiates hosted checkout through backend, links personal-hub records via `payment_ref_id`, persists saved bill/policy upserts, and reconciles status via backend `verify/status` before UI confirmation | `Wired` | Keep; manual QA callback timing across providers still required |
| `creditCardScreen` / `paypalScreen` | Booking payment checkout | Replaced direct `payments` inserts with hosted ExpressPay initiation + reconciliation; booking payment status now follows gateway status (`pending`/`paid`/`failed`) | `Wired` | Keep; consider UI simplification (hosted checkout means no local credential capture needed) in future UX pass |
...
| `editProfileScreen` | Profile fields + save | Loads core profile fields plus persisted profile preferences, updates full profile payload in `profiles`, uploads/removes avatar via storage-backed `avatar_url`, and shows deterministic gradient avatar fallback when no custom photo is set | `Wired` | Keep; if desired, move avatar uploads from shared `attachments` bucket to a dedicated profile-avatar bucket |
