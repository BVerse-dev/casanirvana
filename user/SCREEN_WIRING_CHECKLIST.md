...
| `paymentMethodScreen` and sub-screens | Method selection and checkout transition | Enforces one canonical personal-hub payload contract; mobile-money path receives full typed payload, provider identifiers (`provider` + `providerId`), and save toggles (`saveAccount` / `savePolicy`) | `Wired` | Keep; personal-hub transactions are intentionally constrained to supported payment method path |
| `mobileMoneyScreen` | Payment initiation + confirmation modal | Creates pending `payments` + domain transaction rows, persists saved bill/policy records via upsert when enabled, displays pending confirmation (no fake success timer), and now persists `provider_id` where supported | `Wired` | Keep; finalize asynchronous provider callback/webhook flow in backend before enabling completed-status UX |
