-- Phase 35: Community directory membership integrity hardening
-- Purpose:
--   - Ensure resident and tenant profiles always have a same-community directory row.
--   - Repair active membership rows that drifted away from profiles.community_id.
--   - Install profile-driven sync hooks so direct profile edits keep the canonical
--     community directory source of truth aligned across Guard, User, and Superadmin.
...
create table if not exists public.datafix_phase35_community_membership_repair_backup (
...
create table if not exists public.datafix_phase35_community_membership_insert_backup (
  backup_tag text not null,
...
backup_reactivations as (
  insert into public.datafix_phase35_community_membership_repair_backup (
    backup_tag,
    backup_reason,
    id,
    community_id,
  ...
  select
    'phase35_community_directory_membership_integrity_20260307',
...
insert into public.datafix_phase35_community_membership_insert_backup (
  backup_tag,
  insert_reason,
  id,
  community_id,
