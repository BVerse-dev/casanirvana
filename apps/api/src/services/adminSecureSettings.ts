...
const SECURITY_PRIVACY_DEFAULTS = {
  terms_url: 'https://casanirvana.app/terms-of-service/',
  privacy_url: 'https://casanirvana.app/privacy-policy/',
  refund_policy_url: '',
...
export async function sendAdminConfiguredEmail(input: {
  to: string;
  replyTo?: string;
  subject: string;
  text: string;
  ...
  const settings = await loadSettings('smtp', SMTP_DEFAULTS);
  if (!settings.smtp_host || !settings.smtp_from_email || !settings.smtp_username || !settings.smtp_password) {
    throw new Error('SMTP is not configured');
  }

  const host = String(settings.smtp_host);
  const port = Number(settings.smtp_port || 587);
  const timeoutMs = Math.max(1000, Number(settings.smtp_timeout || 30) * 1000);
  const encryption = String(settings.smtp_encryption || '').toLowerCase();
  const secure = encryption === 'ssl' || settings.smtp_enable_ssl === true || port === 465;
  const requireTLS = encryption === 'tls' || encryption === 'starttls' || settings.smtp_enable_tls === true;
  const transporter = nodemailer.createTransport({
    host,
    port,
    secure,
    requireTLS,
    auth: { user: String(settings.smtp_username), pass: String(settings.smtp_password) },
    connectionTimeout: timeoutMs,
    greetingTimeout: timeoutMs,
    socketTimeout: timeoutMs,
  });
  ...
  await transporter.sendMail({
    from: { name: String(settings.smtp_from_name || 'Casa Nirvana'), address: String(settings.smtp_from_email) },
    to: input.to,
