...
export function requireMarketingContactApiKey(req: Request, res: Response, next: NextFunction) {
  const expectedKey = process.env.MARKETING_CONTACT_API_KEY;
  const providedKey = req.header('x-marketing-contact-key');

  if (!expectedKey) {
    return next(
      createHttpError(500, 'MARKETING_CONTACT_API_KEY_MISSING', 'Marketing contact API key is not configured')
    );
  }

  if (!providedKey || providedKey !== expectedKey) {
    return next(createHttpError(401, 'MARKETING_CONTACT_API_KEY_INVALID', 'Invalid marketing contact API key'));
  }
