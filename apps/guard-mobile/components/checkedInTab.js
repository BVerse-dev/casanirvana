...
const CheckedInTab = ({ navigation }) => {
  ...
  const label = (key, fallback) => {
    const value = tr(key);
    return value === `checkedInTab:${key}` ? fallback : value;
  };

  const mappedPasses = (passes || []).map((pass) => mapPassToInOutItem(pass, 'checked_in'));
  ...
  if (mappedPasses.length === 0) {
    return (
      <View style={styles.stateContainer}>
        ...
        <Text style={{ ...Fonts.Medium14grey, marginTop: Default.fixPadding }}>
          {label('noVisitorsInside', 'No visitors currently inside')}
        </Text>
      </View>
    );
  }
