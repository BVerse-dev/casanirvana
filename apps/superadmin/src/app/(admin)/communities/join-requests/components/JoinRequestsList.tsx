...
import { type JoinRequest, type JoinRequestStatus, useListJoinRequests, usePendingJoinRequestsCount, useUpdateJoinRequest } from "@/hooks/useJoinRequests";
import { useRouter, useSearchParams } from "next/navigation";
import { FormEvent, useState } from "react";
import { Alert, Badge, Button, Card, CardBody, CardFooter, CardHeader, Col, Form, Modal, ModalBody, ModalFooter, ModalHeader, Row } from "react-bootstrap";
...
const JoinRequestsList = () => {
  ...
  const pendingCount = usePendingJoinRequestsCount();
    ...
    <Card>
      <CardHeader className="border-bottom">
        <div className="d-flex flex-column flex-lg-row justify-content-between gap-3">
          ...
          <div className="d-flex align-items-center gap-2"><Badge bg="warning">{pendingCount.data || 0} pending</Badge></div>
        </div>
        ...
        {query.isLoading ? (
          ...
          <div className="d-flex flex-column gap-3">
            {payload.data.map((request) => (
              <div className="border rounded p-3" key={request.id}>
                <Row className="align-items-center g-3">
                  <Col xl={3}><h5 className="mb-1">{request.full_name || "Resident name unavailable"}</h5><p className="text-muted mb-0">{request.email || request.phone || "Contact unavailable"}</p></Col>
                  ...
                  <Col xl={2}><Badge bg={statusVariant(request.status)}>{statusLabel(request.status)}</Badge><small className="text-muted d-block mt-2">Submitted {formatDate(request.created_at)}</small></Col>
                  <Col xl={4}><div className="d-flex flex-wrap justify-content-xl-end gap-2"><Button variant="light" size="sm" onClick={() => openDetails(request)}><IconifyIcon icon="ri:eye-line" className="me-1" />Details</Button>{(request.status === "pending" || request.status === "pending_manual_review") && <><Button variant="success" size="sm" disabled={updateRequest.isPending} onClick={() => openDecision(request, "approved")}>Approve</Button><Button variant="outline-danger" size="sm" disabled={updateRequest.isPending} onClick={() => openDecision(request, "rejected")}>Reject</Button></>}</div></Col>
                </Row>
              </div>
            ))}
          </div>
