( function( $ ) {
    ...
    $( window ).on( 'elementor/frontend/init', function() {
        ...
        elementorFrontend.hooks.addAction( 'frontend/element_ready/pxl_video_button_carousel.default', function( $scope ) {
            pxl_swiper_handler($scope);
        } );
        ...
        $scope.find('.pxl-swiper-slider').each(function(index, element) {
            ...
            if(settings['autoplay'] || settings['autoplay'] === 'true'){
                carousel_settings['autoplay'] = {
                    ...
                    reverseDirection: !!settings['reverse_autoplay']
                };
            } else {
            ...
            // Nếu là .pxl-client-carousel1.style-6 thì xử lý class 'is-middle'
            if ($this.closest('.pxl-client-carousel1.style-6').length > 0) {
                function updateMiddleItem() {
                    const visibleItems = $this.find('.swiper-slide-visible');
                    visibleItems.removeClass('is-middle');
                    if (visibleItems.length > 0) {
                        const middleIndex = Math.floor(visibleItems.length / 2);
                        visibleItems.eq(middleIndex).addClass('is-middle');
                    }
                }

                swiper.on('slideChange', updateMiddleItem);
                swiper.on('resize', updateMiddleItem);
                swiper.on('transitionEnd', updateMiddleItem);
