...
const UnitAddCard = ({ formData }: UnitAddCardProps) => {
  // Use form data or fallback to defaults
  const unitNumber = formData?.unit_number || "Sample Unit";
  const unitType = formData?.type?.toUpperCase() || "2 BHK";
  const area = formData?.area || 1200;
  const floor = formData?.floor || 3;
  const status = formData?.status || "vacant";
  const rentAmount = formData?.rent_amount || 25000;
  ...
  const displayAmenities = amenities.length > 0 ? amenities : [
    { name: "Parking", color: "success" },
    { name: "Gym Access", color: "info" },
    { name: "Garden View", color: "warning" },
    { name: "24/7 Security", color: "primary" },
  ];
      ...
      <Card>
        <CardBody>
          ...
          <div className="mt-3">
            ...
            <p className="mb-1">Casa Nirvana Community, Bangalore</p>
            <h5 className="text-dark fw-medium mt-3">Monthly Rent :</h5>
            <h4 className="fw-semibold mt-2 text-muted">${rentAmount?.toLocaleString()}</h4>
          </div>
            ...
            <div className="d-flex flex-wrap gap-2">
              ...
              {displayAmenities.length > 4 && (
                <span className="badge bg-secondary-subtle text-secondary">
                  +{displayAmenities.length - 4} more
                </span>
              )}
            </div>
          </div>
