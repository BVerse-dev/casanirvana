...
const InsuranceDistributionChart = () => {
  ...
  const options: ApexOptions = {
    ...
    tooltip: {
      y: {
        formatter: (value: number) => `${value.toLocaleString('en-GH')} payments`,
      },
    },
  };
  ...
  return (
    <Card className="mb-3">
      <Card.Header>
        <Card.Title className="mb-0">Provider Distribution</Card.Title>
      </Card.Header>
      <Card.Body>
        {loading && transactions.length === 0 ? (
          <div className="text-center py-5">
            <Spinner animation="border" size="sm" className="me-2" />
            Loading insurance provider mix...
          </div>
        ) : error ? (
          <Alert variant="danger" className="mb-0">{error}</Alert>
        ) : providerRows.length === 0 ? (
          <div className="py-5 text-center text-muted">No insurance payment distribution is available yet.</div>
        ) : (
          <>
            <div className="text-center mb-3">
              <p className="text-muted mb-1">Distribution of insurance payments by synced provider</p>
            </div>
            <ReactApexChart options={options} series={providerRows.map((row) => row.count)} type="donut" height={320} className="apex-charts" />
            <div className="mt-3">
              <h5 className="font-14 mb-2">Premium Value by Provider</h5>
