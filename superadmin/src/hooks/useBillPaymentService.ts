...
export const useBillPaymentService = () => {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      setMetrics({
        ...
        growth: { transactions: 8.1, revenue: 6.7, successRate: 0.4 }
      })

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch bill payment metrics')
    } finally {
      setLoading(false)
    }
  }
