...
const transformFromDB = (data: any): AgencyConfiguration => {
  return {
    ...
    hot_lead_budget_range: data.hot_lead_budget_range || 'GH₵ 50k-100k',
...
const transformToDB = (data: CreateAgencyConfigurationData | UpdateAgencyConfigurationData) => {
  ...
  const commission = uiData.commission_settings;
  const listing = uiData.listing_settings;
  const leads = uiData.lead_settings;
  const communication = uiData.communication;
  const performance = uiData.performance;
  const financial = uiData.financial;
  const compliance = uiData.compliance;

  return {
    ...data,
    // Ensure proper field mapping and defaults
    default_rate: data.default_rate || commission?.default_rate || 2.5,
    residential_rate: data.residential_rate || commission?.property_type_rates?.residential || 2.0,
    commercial_rate: data.commercial_rate || commission?.property_type_rates?.commercial || 3.0,
    luxury_rate: data.luxury_rate || commission?.property_type_rates?.luxury || 3.5,
    plot_rate: data.plot_rate || commission?.property_type_rates?.plot || 1.5,
    junior_agent_split: data.junior_agent_split || commission?.agent_tier_rates?.junior || 40,
    senior_agent_split: data.senior_agent_split || commission?.agent_tier_rates?.senior || 50,
    team_leader_split: data.team_leader_split || commission?.agent_tier_rates?.team_leader || 60,
    manager_split: data.manager_split || commission?.agent_tier_rates?.manager || 70,
    split_policy: data.split_policy || commission?.split_policy || 'tiered',
    payment_schedule: data.payment_schedule || commission?.payment_schedule || 'monthly',
    auto_approval_required: data.auto_approval_required ?? listing?.auto_approval_required ?? true,
    max_photos_per_listing: data.max_photos_per_listing || listing?.max_photos_per_listing || 20,
    listing_duration_days: data.listing_duration_days || listing?.listing_duration_days || 90,
    renewal_notification_days: data.renewal_notification_days || listing?.renewal_notification_days || 7,
    featured_listing_fee: data.featured_listing_fee || listing?.featured_listing_fee || 5000,
    mandatory_fields: data.mandatory_fields || listing?.mandatory_fields || ['title', 'price', 'location', 'area', 'property_type'],
    auto_assignment: data.auto_assignment ?? leads?.auto_assignment ?? true,
    lead_rotation: data.lead_rotation || leads?.lead_rotation || 'performance_based',
    follow_up_reminders: data.follow_up_reminders ?? leads?.follow_up_reminders ?? true,
    max_leads_per_agent: data.max_leads_per_agent || leads?.max_leads_per_agent || 25,
    lead_expiry_days: data.lead_expiry_days || leads?.lead_expiry_days || 30,
    hot_lead_budget_range: data.hot_lead_budget_range || leads?.hot_lead_criteria?.budget_range || 'GH₵ 50k-100k',
    hot_lead_response_time_hours: data.hot_lead_response_time_hours || leads?.hot_lead_criteria?.response_time_hours || 2,
    hot_lead_engagement_score: data.hot_lead_engagement_score || leads?.hot_lead_criteria?.engagement_score || 80,
    sms_notifications: data.sms_notifications ?? communication?.sms_notifications ?? true,
    email_notifications: data.email_notifications ?? communication?.email_notifications ?? true,
    whatsapp_integration: data.whatsapp_integration ?? communication?.whatsapp_integration ?? true,
    automated_follow_ups: data.automated_follow_ups ?? communication?.automated_follow_ups ?? true,
    client_portal_access: data.client_portal_access ?? communication?.client_portal_access ?? true,
    marketing_emails: data.marketing_emails ?? communication?.marketing_emails ?? false,
    monthly_listing_target: data.monthly_listing_target || performance?.target_settings?.monthly_listing_target || 10,
    monthly_deal_target: data.monthly_deal_target || performance?.target_settings?.monthly_deal_target || 3,
    monthly_revenue_target: data.monthly_revenue_target || performance?.target_settings?.monthly_revenue_target || 500000,
    track_response_time: data.track_response_time ?? performance?.kpi_tracking?.response_time ?? true,
    track_conversion_rate: data.track_conversion_rate ?? performance?.kpi_tracking?.conversion_rate ?? true,
    track_client_satisfaction: data.track_client_satisfaction ?? performance?.kpi_tracking?.client_satisfaction ?? true,
    track_repeat_business: data.track_repeat_business ?? performance?.kpi_tracking?.repeat_business ?? true,
    performance_bonus_enabled: data.performance_bonus_enabled ?? performance?.incentive_structure?.performance_bonus ?? true,
    quarterly_incentives_enabled: data.quarterly_incentives_enabled ?? performance?.incentive_structure?.quarterly_incentives ?? true,
    annual_awards_enabled: data.annual_awards_enabled ?? performance?.incentive_structure?.annual_awards ?? true,
    client_payment_days: data.client_payment_days || financial?.payment_terms?.client_payment_days || 30,
    commission_payment_days: data.commission_payment_days || financial?.payment_terms?.commission_payment_days || 15,
    late_payment_penalty: data.late_payment_penalty || financial?.payment_terms?.late_payment_penalty || 2,
    marketing_budget: data.marketing_budget || financial?.expense_management?.marketing_budget || 50000,
    travel_allowance: data.travel_allowance || financial?.expense_management?.travel_allowance || 5000,
    communication_allowance: data.communication_allowance || financial?.expense_management?.communication_allowance || 2000,
    gst_applicable: data.gst_applicable ?? financial?.tax_settings?.gst_applicable ?? true,
    gst_percentage: data.gst_percentage || financial?.tax_settings?.gst_percentage || 15,
    gst_number: data.gst_number ?? financial?.tax_settings?.gst_number ?? '',
    tds_applicable: data.tds_applicable ?? financial?.tax_settings?.tds_applicable ?? true,
    tds_percentage: data.tds_percentage || financial?.tax_settings?.tds_percentage || 5,
    rera_registration_mandatory: data.rera_registration_mandatory ?? compliance?.rera_compliance?.registration_mandatory ?? true,
    rera_document_verification: data.rera_document_verification ?? compliance?.rera_compliance?.document_verification ?? true,
    rera_periodic_renewal: data.rera_periodic_renewal ?? compliance?.rera_compliance?.periodic_renewal ?? true,
    client_data_encryption: data.client_data_encryption ?? compliance?.data_protection?.client_data_encryption ?? true,
    gdpr_compliance: data.gdpr_compliance ?? compliance?.data_protection?.gdpr_compliance ?? true,
    data_retention_months: data.data_retention_months || compliance?.data_protection?.data_retention_months || 60,
    agreement_templates: data.agreement_templates ?? compliance?.legal_documentation?.agreement_templates ?? true,
