...
export const useMoneyTransferService = () => {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      const { data: providerData, error: providerError } = await supabase
        ...
        .select('*')
        .eq('service_type', 'money_transfer')
      ...
      setMetrics({
        ...
        growth: { transactions: 5.4, revenue: 11.2, successRate: 0.1 }
      })

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch money transfer metrics')
    } finally {
      setLoading(false)
    }
  }
